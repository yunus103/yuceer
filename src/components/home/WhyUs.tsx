        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 text-center">
          {features.map((feature, index) => (
            <ScrollAnimation key={index} delay={index * 0.1} className="flex flex-col items-center group">
              <div className="w-16 h-16 mb-6 text-[#c05e3e] group-hover:scale-110 transition-transform duration-300">
                <feature.icon strokeWidth={1.5} className="w-full h-full" />
              </div>
              <h3 className="text-sm font-bold tracking-widest uppercase mb-4 text-gray-900 border-b-2 border-transparent group-hover:border-[#c05e3e] pb-2 transition-all">
                {feature.title}
              </h3>
